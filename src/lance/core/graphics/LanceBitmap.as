package lance.core.graphics 
{
	 ...
	 * @author SSen
	 */
	public class LanceBitmap extends Bitmap 
	{
		/** 화면에 출력될 BitmapData */
		...
		/** Thumnail 의 위치 Grid */
		protected var _thumbGrid : InnerGrid;
		/** width */
		 ...
		 * @param width width
		 * @param height height
		 * @param thumbGrid Thumnail 의 위치 Grid
		 ...
		 */
		public function LanceBitmap(source : BitmapData, 
									width : int, 
									height : int, 
									...
									)
		{
			...
			_width = width;
			_height = height;
			_thumbGrid = (thumbGrid == null) ? new InnerGrid() : thumbGrid;
		...
		/** @copy #_thumGrid */
		public function get thumbGrid() : InnerGrid
		{
			return _thumbGrid.clone();
		}
		public function set thumbGrid(grid : InnerGrid) : void
		{
			_thumbGrid = grid;
			draw();
		}
		...
		private function drawThum(bitmapData : BitmapData, position : String) : void
		{
			if (bitmapData != null) {
				...
				if (_width > w + _thumbGrid.left + _thumbGrid.right && _height > h + _thumbGrid.top + _thumbGrid.bottom) { 
					var point : Point = _thumbGrid.getPosition(position, _width, _height, w, h);
