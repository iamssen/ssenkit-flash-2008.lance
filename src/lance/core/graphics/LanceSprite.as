package lance.core.graphics
{
	 ...
	 * @author SSen
	 */
	public class LanceSprite extends Sprite 
	{
		private var _containerMode : Boolean;
		...
		public function addChildTo(child : DisplayObject, dummy : DisplayObject, ... option) : void
		{
			if (contains(dummy)) {
				...
				throw new Error("LanceContainer#addChildTo : dummy 는 container 의 자식이 아닙니다.");
			}
		}
		...
		/** 컨테이너 용도로 사용되는지 여부 */
		public function get containerMode() : Boolean
		{
			return _containerMode;
		}
		public function set containerMode(containerMode : Boolean) : void
		{
			if (_containerMode != containerMode) {
				_containerMode = containerMode;
				if (_containerMode) {
					addEventListener(FocusEvent.FOCUS_IN, focusToChild, false, 0, true); 
				} else {
					removeEventListener(FocusEvent.FOCUS_IN, focusToChild);
				}
			}
		}
		private function focusToChild(event : FocusEvent) : void
		{
			if (numChildren > 0 && stage.focus == this) {
				var f : int;
				for (f = 0;f < numChildren; f++) {
					if (getChildAt(f) is InteractiveObject) {
						stage.focus = getChildAt(f) as InteractiveObject;
						break;
					}
				}
			}
		 ...
		 */
		protected function objectToDisplayObject(obj : Object) : DisplayObject
		{
			var bm : Bitmap;
			if (obj is DisplayObject) { 
				...
				throw new Error("obj 는 displayObject 이거나 bitmapData 여야 합니다.");
			}
		}
		/**
		 ...
		 * @param obj 테스트할 객체
		 */
		protected function isDisplayObjectOrBitmapData(obj : Object) : Boolean
		{
